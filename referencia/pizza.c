...
int quantidade_pizzas(FILE *arq){
	fseek(arq, 0, SEEK_END);
	int tam = trunc(ftell(arq) / tamanho_pizza_bytes());
	return tam;
}

void imprimir_cardapio(FILE *arq, int quantidade_pizzas){
	rewind(arq);
	for(int i = 0; i < quantidade_pizzas; i++){
		TPizza *pizzaTeste = le_pizza(arq);
		imprime_pizza(pizzaTeste);
	}
}
